...
{	
  ...
  */
  ...
  public:
	...
	//! get internal rigid body object
	inline btRigidBody* getRigidBody()
	{		
		return rigidBody;
	}

	//! get internal scene node object
	inline ISceneNode* const getSceneNode()
	{
		return sceneNode;
	}

	//! 인자로 넘겨준 바디가 내 바디인지 검사
	inline bool isMyBody(void *co)
	{
		if(rigidBody == co)
			return true;
		return false;
	...
	//! 인자로 넘겨준 바디쌍중에서 내 바디를 반환 없으면 NULL
	inline btRigidBody *selectMyBody(void *co1,void *co2)
	{
		if(rigidBody == co1 || rigidBody == co2)
			return rigidBody;
  ...
  protected:
    ...
    void SerializeIrrMesh(io::IAttributes* out, io::SAttributeReadWriteOptions* options=0) const;

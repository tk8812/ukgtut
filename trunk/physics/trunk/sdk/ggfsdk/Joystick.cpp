...
//    WCHAR* strCmdLine;
//    int nNumArgs;
//    WCHAR** pstrArgList = CommandLineToArgvW( GetCommandLineW(), &nNumArgs );
...
// "IG_" (ex. "VID_045E&PID_028E&IG_00").  If it does, then it뭩 an XInput device
...
HRESULT SetupForIsXInputDevice()
{
    ...
    for( ;; )
    {
        ...
        for( iDevice=0; iDevice<uReturned; iDevice++ )
        {
            ...
            {
                // Check if the device ID contains "IG_".  If it does, then it뭩 an XInput device
                // Unfortunately this information can not be found by just using DirectInput 
...
HRESULT UpdateInputState( HWND hDlg )
{
	...
	for( int i = 0; i < 8; i++ )
    {
       ...
       /* if ( js.rgbbuttons[i] & 0x80 )
        {
            tchar sz[128];
            stringcchggf::irr_util::DebugOutputFmt(NULL, sz, 128, text("%02d "), i );
            stringcchcat( strtext, 512, sz );
        }*/
    }

    // Axes
    //StringCchggf::irr_util::DebugOutputFmt(NULL, strText, 512, TEXT("%ld"), js.lX ); 
    //SetWindowText( GetDlgItem( hDlg, IDC_X_AXIS ), strText );
    //StringCchggf::irr_util::DebugOutputFmt(NULL, strText, 512, TEXT("%ld"), js.lY ); 
    //SetWindowText( GetDlgItem( hDlg, IDC_Y_AXIS ), strText );
    //StringCchggf::irr_util::DebugOutputFmt(NULL, strText, 512, TEXT("%ld"), js.lZ ); 
    //SetWindowText( GetDlgItem( hDlg, IDC_Z_AXIS ), strText );
    //StringCchggf::irr_util::DebugOutputFmt(NULL, strText, 512, TEXT("%ld"), js.lRx ); 
    //SetWindowText( GetDlgItem( hDlg, IDC_X_ROT ), strText );
    //StringCchggf::irr_util::DebugOutputFmt(NULL, strText, 512, TEXT("%ld"), js.lRy ); 
    //SetWindowText( GetDlgItem( hDlg, IDC_Y_ROT ), strText );
    //StringCchggf::irr_util::DebugOutputFmt(NULL, strText, 512, TEXT("%ld"), js.lRz ); 
    //SetWindowText( GetDlgItem( hDlg, IDC_Z_ROT ), strText );

    //// Slider controls
    //StringCchggf::irr_util::DebugOutputFmt(NULL, strText, 512, TEXT("%ld"), js.rglSlider[0] ); 
    //SetWindowText( GetDlgItem( hDlg, IDC_SLIDER0 ), strText );
    //StringCchggf::irr_util::DebugOutputFmt(NULL, strText, 512, TEXT("%ld"), js.rglSlider[1] ); 
    //SetWindowText( GetDlgItem( hDlg, IDC_SLIDER1 ), strText );

    //// Points of view
    //StringCchggf::irr_util::DebugOutputFmt(NULL, strText, 512, TEXT("%ld"), js.rgdwPOV[0] );
    //SetWindowText( GetDlgItem( hDlg, IDC_POV0 ), strText );
    //StringCchggf::irr_util::DebugOutputFmt(NULL, strText, 512, TEXT("%ld"), js.rgdwPOV[1] );
    //SetWindowText( GetDlgItem( hDlg, IDC_POV1 ), strText );
    //StringCchggf::irr_util::DebugOutputFmt(NULL, strText, 512, TEXT("%ld"), js.rgdwPOV[2] );
    //SetWindowText( GetDlgItem( hDlg, IDC_POV2 ), strText );
    //StringCchggf::irr_util::DebugOutputFmt(NULL, strText, 512, TEXT("%ld"), js.rgdwPOV[3] );

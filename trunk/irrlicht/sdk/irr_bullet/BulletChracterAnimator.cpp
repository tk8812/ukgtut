...
void CBulletChracterAnimator::preStep()
{
	...
	m_rayTarget[0] = m_raySource[0] + down * (geometry.Capsule.hight*.5f + geometry.Capsule.radius) * btScalar(1.1); // / 2.f) * btScalar(1.1);
...
void CBulletChracterAnimator::controlStep_Walker(btScalar Angle, btScalar Speed)
{
	...
	*/
	...
	//	m_rayTarget[0] = m_raySource[0] + down * (geometry.Capsule.hight*.5f + geometry.Capsule.radius )* btScalar(1.1);///2.f) * btScalar(1.5f);
	//	m_rayTarget[1] = m_raySource[1] + forward * (geometry.Capsule.radius ) * btScalar(1.1);		
...

	{
		...
		{
			if(velocity_Y < 0) //현제 더떨어지고있는가?
			{
				velocity_Y = 0; //지상에 이미 착륙했으므로 더 떨어질수없다.
			}			
		}	

...
//------------------------------------------------------------------------------
//! Convert irrlicht to bullet
SIMD_FORCE_INLINE void Irrlicht2Bullet(irr::core::vector3df &in,btVector3 &out)
{
	//out.setValue(in.X,in.Y,in.Z,0);  
	out.setValue(in.X,in.Y,in.Z);  
}

SIMD_FORCE_INLINE void Irrlicht2Bullet(irr::core::quaternion &in,btQuaternion &out)
{
	out.setValue(in.X,in.Y,in.Z,in.W);  	
}

//이건 100%검증안됨..
SIMD_FORCE_INLINE void Irrlicht2Bullet(irr::core::matrix4 &in,btTransform &out)
{
	out.getBasis()[0].setValue(in[0],in[4],in[8]);
	out.getBasis()[1].setValue(in[1],in[5],in[9]);
	out.getBasis()[2].setValue(in[2],in[6],in[10]);
}

//------------------------------------------------------------------------------
//! Convert bullet to irrlicht 
SIMD_FORCE_INLINE void Bullet2Irrlicht(btVector3 &in,irr::core::vector3df &out)									   
{
	out = irr::core::vector3df(in.getX(),in.getY(),in.getZ());
}

SIMD_FORCE_INLINE void Bullet2Irrlicht(btQuaternion &in,irr::core::quaternion &out)
{
	out = irr::core::quaternion(in.getX(),in.getY(),in.getZ(),in.getW());
}
